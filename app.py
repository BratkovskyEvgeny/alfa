...
if st.button("Жэстачайшая правда о нетворкинге"):
    ...
    st.markdown(f"<h6 style='text-align: justify;'>Довожу до сведения, что нетворкинг продвигается продуктивно, отряд не потерял бойцов, все заряжены на изучение вражеских алгоритмов, чтобы принести пользу банку.Допросы, т.е. опросы, все успешно прошли :sunglasses:. Поэтому ниже специально для нетворкеров песня о тяжелой подготовке из культового фильма "Кровавый спорт"</h6>", unsafe_allow_html=True)
